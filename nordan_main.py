...
BOT_TOKEN        = os.getenv("BOT_TOKEN", "8857322700:AAEVZ7bLlidd4OR7TWoyAYZKnXyJQ9laA-0")
ADMIN_ID         = 7438622949
ADMIN_ID2        = 0
ADMIN_IDS        = (ADMIN_ID, ADMIN_ID2)
GROUP_CHAT_ID    = -1004304322155
GROUP_LINK       = "https://t.me/+LywsBj3lhaU4Y2I0"
SUPPORT_LINK     = "https://t.me/+LywsBj3lhaU4Y2I0dj"
BOT_USERNAME     = "Nordansms_bot"
CHANNEL_LINK     = "https://t.me/+tc9J6sWP7vljN2M0"
GROUP_JOIN_LINK  = "https://t.me/+LywsBj3lhaU4Y2I0"

# Membership checks require a channel/group username or numeric chat ID.
# Invite links above are used for the join buttons.
REQUIRED_CHANNEL = "-1004317167379"
REQUIRED_GROUP   = "-1004304322155"

# ── Nordan SMS API
NORDAN_URL    = "https://nordansms.com/restapi/smsreport"
NORDAN_TOKEN  = os.getenv("NORDAN_TOKEN", "nat_db3c0adf8949a917c4d27ba9e9463f234220725f90d026ad4a2b6cb4361c4016")
FETCH_LIMIT   = 100
...
SERVICE_ICONS = {
    ...
    "YOUTUBE":"▶️","GOOGLE":"🔍","TWITTER":"🐦","TIKTOK":"🎵","SNAPCHAT":"👻",
    "AMAZON":"📦","PAYPAL":"💳","MICROSOFT":"🪟","APPLE":"🍎",
    "NETFLIX":"🎬","DISCORD":"🎮","UBER":"🚗","LINKEDIN":"💼",
    "IMO":"💬","VIBER":"📞","SIGNAL":"🔒","ZOOM":"📹","SKYPE":"🔵",
    "AIRBNB":"🏠","BINANCE":"🟡","COINBASE":"🔵","STEAM":"🎮",
}

# The first emoji is used beside the service name and the second is used
# as the coloured marker on the inline buttons.
SERVICE_COLORS = {
    "WHATSAPP":"🟩", "FACEBOOK":"🟦", "INSTAGRAM":"🟪",
    "TELEGRAM":"🔷", "YOUTUBE":"🟥", "GOOGLE":"🟥", "TWITTER":"🟦",
    "TIKTOK":"⬛", "SNAPCHAT":"🟨", "AMAZON":"🟧",
    "PAYPAL":"🟦", "MICROSOFT":"🟦", "APPLE":"⬜",
    "NETFLIX":"🟥", "DISCORD":"🟪", "UBER":"⬛",
    "LINKEDIN":"🟦",
}
...
def service_icon(service: str) -> str:
    return SERVICE_ICONS.get(str(service).strip().upper(), "📨")

def service_color(service: str) -> str:
    return SERVICE_COLORS.get(str(service).strip().upper(), "🟦")

def service_menu_label(service: str, count: int, price: float) -> str:
    """Label used by the service picker, including its colour marker."""
    return f"{service_color(service)} {service_icon(service)} {str(service).strip()}  ·  {count} avail  ·  ${price:.4f}"
...
def group_card(country: str, ctry_flag: str, service: str, svc_icon: str, masked_num: str, otp_text: str) -> str:
    ...
    return (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{ctry_flag}  <b>{escape(country)}</b>\n"
        f"{service_color(service)} {svc_icon}  <b>{escape(service)}</b>\n"
...
def private_card(country: str, ctry_flag: str, service: str, svc_icon: str, full_num: str, otp_text: str, reward: float, balance: float) -> str:
    ...
    return (
        f"✅ <b>OTP DELIVERED!</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{ctry_flag}  <b>{escape(country)}</b>   {service_color(service)} {svc_icon}  <b>{escape(service)}</b>\n"
...
async def forward_sms(bot, sms: dict):
    """Deliver a new SMS to its assigned user and the OTP group."""
    raw_num = str(sms.get("num", "")).strip()
    message = str(sms.get("message", "")).strip()
    if not raw_num or not message:
        return

    number = re.sub(r"\D", "", raw_num)
    otp_text = extract_otp(message)
    if not otp_text:
        logger.warning(f"SMS received without an OTP: {mask(number)}")
        return

    record = get_number_by_value(number)
    if not record or record.get("status") != "assigned" or not record.get("assigned_to"):
        logger.info(f"OTP ignored because number is not assigned: {mask(number)}")
        return

    uid = int(record["assigned_to"])
    country = str(record.get("country") or "").strip()
    service = str(record.get("service") or "").strip() or detect_service(
        str(sms.get("cli", "")), message
    )

    ctry_flag = flag(country)
    if ctry_flag == "🌍":
        detected_country, detected_flag = country_from_number(number)
        if not country or country.lower() == "unknown":
            country = detected_country
        ctry_flag = detected_flag
    svc_icon = service_icon(service)

    save_otp(number, country, service, otp_text, uid, message)
    ...
    try:
        await bot.send_message(
            uid,
            private_card(
                country, ctry_flag, service, svc_icon,
                number, otp_text, reward, balance
            ),
            parse_mode="HTML",
        )
    except Exception as exc:
        logger.warning(f"Could not deliver OTP privately to {uid}: {exc}")
    ...
    if GROUP_CHAT_ID:
        try:
            await bot.send_message(
                ...
                group_card(
                    country, ctry_flag, service, svc_icon,
                    mask(number), otp_text
                ),
                parse_mode="HTML",
            )
        except Exception as exc:
            logger.warning(f"Could not post OTP to group: {exc}")

    mark_number_used(number)
    logger.info(f"Delivered {service} OTP for {country} to user {uid}")
